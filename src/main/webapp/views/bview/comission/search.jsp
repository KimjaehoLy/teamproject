		...
		$(".bb").click(function() {
						...
						$('.del').click(function() {
											var radioVal = $(
													...
													.val()
											location.href = "/template/bview/comission/deleteReg?order_num2="
													+ radioVal
										});
						...
						$(".chks1").click(function() {
							...
							$.ajax({
								type : "GET",
								url : "/ajax/eview/marketing/chkData2",
								data : {
									order_num2 : chk1,
									crre_name : chk2
								},
...
<input type="hidden" name="lev"
	value="<%=(((UserVO) session.getAttribute("mydata")).getUser_lev())%>" />
<h2>영업관리</h2>
<hr />
<div class="FFFF">
	<form action="/template/bview/comission/search">
		<div class="col-lg-14 input-group mb-3">
			주문번호&nbsp;<input type="search" class="form-control" name="order_num2" /><input
				type="submit" class="btn btn-outline-info" value="조회" />
			&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
			고객&nbsp;<input type="search" class="form-control" name="customer" /><input
				type="submit" class="btn btn-outline-info" value="조회" />	
		</div>
	</form>
</div>
	...
	<table class="table">
		<thead class="table-primary">
			<tr>
				...
				<th scope="col" width="10%">수량(kg)</th>
				<th scope="col" width="15%">제품종류</th>
				<th scope="col" width="8%">담당자</th>
				<th scope="col" width="10%">승인여부</th>
				<th scope="col" width="10%">비고</th>
			</tr>
		</thead>
		<c:forEach items="${data }" var="vo" varStatus="i">
			<tr>
				<td><input type="radio" name="chkBox" value="${vo.order_num2 }" /></td>
				<td>${vo.order_num2 }</td>
				<td>${vo.regdate }</td>
				<td>${vo.customer }</td>
				<td>${vo.cnt }</td>
				<td>${vo.kind }</td>
				<td>${vo.leader }</td>
				<c:if test="${mydata.user_lev eq '팀장'}">
					<td><a
						href="/template/bview/comission/modifyReg?order_num2=${vo.order_num2 }">${vo.chk }</a>
					</td>
				</c:if>
				<c:if test="${mydata.user_lev != '팀장'}">
					<td>${vo.chk }</td>
				</c:if>
				<td>${vo.content }</td>
			</tr>
		...
		<tr>
			<td colspan="9"><div class="PPPP">
				<nav aria-label="Page navigation example">
					<ul class="pagination">
						<c:if test="${startPage>1 }">
							<li class="page-item"><a class="page-link"
								href="?nowPage=${startPage-1 }&order_num2=${search1 }&customer=${search2}" aria-label="Previous"> <span
						...
						<c:forEach begin="${startPage }" end="${endPage }" step="1"
							...
							<c:choose>
								...
								<c:otherwise>
									<li class="page-item"><a class="page-link"
										href="?nowPage=${i }&order_num2=${search1 }&customer=${search2}">${i }</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${endPage<lastPage }">
							<li class="page-item"><a class="page-link"
								href="?nowPage=${endPage+1 }&order_num2=${search1 }&customer=${search2}" aria-label="Next"> <span
...
<div class="inform1">
	<form class="input-group mb-3 lst" action="inputReg" name="insertT">
		<div class="col-lg-2">
			의뢰번호 <input type="text" class="form-control oo1" name="order_num2" placeholder="ex) 2020BB0000"/>
		</div>
		<div class="col-lg-2">
			고객 <input type="text" class="form-control cc1" name="customer" placeholder="ex) 이디야"/>
		</div>
		<div class="col-lg-2">
			수량(kg) <input type="number" class="form-control" name="cnt" />
		</div>
		<div class="col-lg-2">
			제품종류 <input type="text" class="form-control" name="kind" />
		</div>
		<div class="col-lg-2">
		...
		<input type="hidden" name="chk" value="진행중" />
		<div>
			<input type="button" class="btn btn-outline-info btn_1 chks1" value="등록" />
			<input class="btn btn-outline-info btn_1" type="reset" value="초기화" />
		</div>
	</form>
</div>
<div class="inform2 off">
	<form class="input-group mb-3 lst" action="updateReg" name="updateT">
		<div class="col-lg-2">
			의뢰번호 <input type="text" class="form-control" name="order_num2"
				id="onum" readonly />
		</div>
		<div class="col-lg-2">
		...
		<div class="col-lg-2">
			수량(kg) <input type="number" class="form-control" name="cnt" id="ct" />
		</div>
		<div class="col-lg-2">
			제품종류 <input type="text" class="form-control" name="kind" id="knd" />
		</div>
		<div class="col-lg-2">
