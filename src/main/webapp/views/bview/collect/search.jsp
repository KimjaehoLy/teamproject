			...
			.ready(
					function() {
								...
								.click(
										function() {
											var radioVal = $(
													...
													.val()
											location.href = "/template/bview/collect/deleteReg?order_num5="
													+ radioVal
										});
						$(".mod").click(function() {
						...
						$(".chks1").click(function() {
							...
							$.ajax({
								type : "GET",
								url : "/ajax/eview/marketing/chkData5",
								data : {
									order_num5 : chk1,
									crre_name : chk2
								},
...
<hr />
<div class="FFFF">
	<form action="/template/bview/collect/search">
		<div class="col-lg-14 input-group mb-3">
			수금번호&nbsp;<input type="text" class="form-control" name="order_num5" /> <input
	...
	<table class="table">
		<thead class="table-primary">
			<tr>
				...
				<th scope="col" width="15%">수금번호</th>
				<th scope="col" width="11%">수금일자</th>
				<th scope="col" width="10%">고객</th>
				<th scope="col" width="10%">수금구분</th>
				<th scope="col" width="10%">증빙번호</th>
				<th scope="col" width="10%">정리잔액</th>
		...
		<c:forEach items="${data }" var="vo">
			<tr>
				<td><input type="radio" name="chkBox" value="${vo.order_num5 }" /></td>
				<td>${vo.order_num5 }</td>
				<td>${vo.regdate }</td>
				<td>${vo.customer }</td>
				<td>${vo.col_kind }</td>
					...
					<ul class="pagination">
						<c:if test="${startPage>1 }">
							<li class="page-item"><a class="page-link"
								href="?nowPage=${startPage-1 }&order_num5=${search1 }&customer=${search2}" aria-label="Previous"> <span
						...
						<c:forEach begin="${startPage }" end="${endPage }" step="1"
							...
							<c:choose>
								...
								<c:otherwise>
									<li class="page-item"><a class="page-link"
										href="?nowPage=${i }&order_num5=${search1 }&customer=${search2}">${i }</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:if test="${endPage<lastPage }">
							<li class="page-item"><a class="page-link"
								href="?nowPage=${endPage+1 }&order_num5=${search1 }&customer=${search2}" aria-label="Next"> <span
...
<div class="inform1">
	<form class="input-group mb-3 lst" action="inputReg" name="insertT">
		<div class="col-lg-2">
			수금번호 <input type="text" class="form-control oo1" name="order_num5" placeholder="ex) 2020DD0000"/>
		</div>
		<div class="col-lg-1">
			고객 <input type="text" class="form-control cc1" name="customer" placeholder="ex) 이디야"/>
		</div>
		<div class="col-lg-2">
			수금구분 <input type="text" class="form-control" name="col_kind" />
		</div>
		<div class="col-lg-2">
			증빙번호 <input type="text" class="form-control" name="evi" />
		</div>
		<div class="col-lg-2">
			정리잔액 <input type="text" class="form-control" name="bal" />
		</div>
		<div class="col-lg-1">
...
<div class="inform2 off">
	<form class="input-group mb-3 lst" action="updateReg" name="updateT">
		<div class="col-lg-2">
			수금번호 <input type="text" class="form-control" name="order_num5"
				readonly id="onum" />
		</div>
		<div class="col-lg-1">
		...
		<div class="col-lg-2">
			수금구분 <input type="text" class="form-control" name="col_kind" id="ck" />
		</div>
		<div class="col-lg-2">
			증빙번호 <input type="text" class="form-control" name="evi" id="ev" />
		</div>
		<div class="col-lg-2">
			정리잔액 <input type="text" class="form-control" name="bal" id="vl" />
		</div>
		<div class="col-lg-1">
